...
**作案手法**：
1. **液氮冷脆**：赵泰利用钢材在-196℃超低温下会变脆的物理特性，先用磨光机削弱钢钎截面，再用液氮冷冻，使钢钎在受力时瞬间脆断。
2. **泥浆比重陷阱**：泥浆比重被顾远调整到1.52（人体比重仅1.02-1.1），严成坠入后无法浮起，被永远埋在泥浆池底。

**动机**：赵泰挪用了800万工程款，被严成发现，为掩盖罪行而杀人灭口。

**关键证据**：断裂钢钎的平整断面（脆断特征）、现场残留的液氮白霜、苏晓捡到的冷脆金属碎片。

**结案陈词**：大桥不说话，但数据会报复；证据会消失，但物理原理永远不会错。"""
